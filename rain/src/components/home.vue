    ...
    <div class="amap-wrapper">
      <el-row class="w1200">
        <el-col :span="5">
          ...
          <el-select placeholder="请选择地区" class="w160" value-key v-model="info.city" @change="changeInfo">
            <el-option
        ...
        <el-col :span="6">
          ...
          <el-select placeholder="请选择地区" class="w160" v-model="info.time" @change="changeInfo">
        ...
        <el-col :span="9">
          ...
          <el-date-picker
            type="daterange"
            placeholder="选择日期范围" v-model="info.historyTime" @change="e => changeTime(e)">
          </el-date-picker>
        </el-col>
      ...
      <el-row class="con">
        <div class="center">
          <el-col>&nbsp;&nbsp;
            <span v-if="info.city != null">{{info.city.areaName}}&gt;&nbsp;</span>
            <span v-if="info.time">实时数据查询:&nbsp;{{info.time}}小时&gt;</span>
            <span v-if="info.historyTime[0]">历史数据查询:&nbsp;{{info.startTime}} — {{info.endTime}}</span>
          </el-col>
        </div>
      </el-row>

      <div class="mapContain" v-loading="loading" element-loading-text="地图正在拼命加载中">
...
export default {
  ...
  created () {
    this.info.startTime = this.info.historyTime[0] = this.timeFilter(new Date(new Date().setMonth((new Date().getMonth()-1))))
    this.info.endTime = this.info.historyTime[1] = this.timeFilter(new Date())
  ...
  methods: {
    changeInfo () {
      this.loading = true
      this.init(this.info.city,this.isShowHeatmap)
    },
    ...
    changeTime (e) {
      // console.log(this.info.historyTime[0]);
      // if (this.info.historyTime != undefined) {
      //   console.log(123);
      //   // this.info.startTime = this.timeFilter(new Date(this.info.historyTime[0]))
      //   // this.info.endTime = this.timeFilter(new Date(this.info.historyTime[1]))
      //   // this.changeInfo()
      // } else {
      //   this.info.startTime = null
      //   this.info.endTime = null
      // }
    }
  }
}
